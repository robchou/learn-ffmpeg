...
void video_refresh_timer(void *userdata) {
    ...
    if (is->video_st) {
        if (is->pictq_size == 0) {
            ...
            /* Make the minimum delay as 0.010 */
...
int stream_component_open(VideoState *is, int stream_index) {
    ...
    if (stream_index < 0 || stream_index >= pFormatCtx->nb_streams) {
        fprintf(stderr, "error: invalid stream index!\n");
        return -1;
    }
...
int decode_thread(void *arg) {
    ...
    if (avformat_open_input(&pFormatCtx, is->filename, NULL, NULL) < 0) {
        fprintf(stderr, "Couldn't not open %s\n", is->filename);
        return -1;
    }
...
int main(int argc, char* argv[])
{
    ...
    if(!is->parse_tid) {
        fprintf(stderr, "error: failed to create decode_thread!\n");
        av_free(is);
        return -1;
    }
    ...
    for (;;) {
        ...
        switch (event.type) {
            ...
            case SDL_QUIT:
                 ...
                 */
                SDL_CondSignal(&is->audioq);
                SDL_CondSignal(&is->videoq);
                SDL_Quit();
                return 0;
                break;
            case FF_ALLOC_EVENT:
